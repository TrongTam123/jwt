...
const getPostById = async (req, res) => {
    try {
        let data = await postApiService.getPost(req.query.id)
        return res.status(200).json({
                EM: data.EM, //error message
                EC: data.EC, // ERROR CODE
                DT: data.DT // Data
...
const getPostCurrent = async (req, res) => {
    try {
        let data = await postApiService.getPostRecent()
        return res.status(200).json({
                EM: data.EM, //error message
                EC: data.EC, // ERROR CODE
                DT: data.DT // Data
...
const getAllPostCurrent = async (req, res) => {
    try {
        if (req.query.page && req.query.limit) {
            let page = req.query.page
            let limit = req.query.limit

            let data = await postApiService.getAllCurrentPost(+page, +limit)
